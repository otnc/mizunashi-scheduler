...
const RULES = [
  ...
  },
  {
    ...
    // target="_blank" の rel 付け忘れは .astro を ESLint 対象にしていない現状では捕まえられない
    kind: 'grep',
    pattern: /<a\s[^>]*href=["']https?:\/\//,
    include: ['apps/web/src/**/*.{tsx,astro}'],
    message: '外部リンクは ExternalLink コンポーネントを使うこと',
  },
];
```

**行末に `invariant-ok(ルールID)` と理由を書くとその行だけ除外できる。** 例外はルールを緩めるのではなくコード側に理由込みで残す。ルールを緩めると検出力がその後ずっと落ちたままになるため。
